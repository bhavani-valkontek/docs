...
# Set the path to Tesseract executable (update this to your installation path)
pytesseract.pytesseract.tesseract_cmd = r'C:\Users\bhavani\AppData\Local\Programs\Tesseract-OCR'  # Windows example
# For Linux: typically '/usr/bin/tesseract'

# Default model path and device selection (CUDA if available)
DEFAULT_MODEL_PATH = "best_new3s.pt"
...
# Function to perform OCR on a detected region
...
def capture_image(width=640, height=640):
...
def process_image(original_img, yolo_model, confidence_threshold, output_dir, csv_path, show_images, start_time):
    ...
    try:
        ...
        resized_input = cv2.resize(original_img, (640, 640))  # Resize image for YOLO model
        ...
        if not boxes or not boxes.xyxy.any():  # Check if no objects are detected
            ...
            for i in range(len(boxes)):  # Loop over each detection box
                ...
                roi = original_img[y1_orig:y2_orig, x1_orig:x2_orig]

                # Perform OCR on the ROI
                detected_text = perform_ocr(roi)

                # If text was detected, append it to the label
                if detected_text:
                    label_with_text = f"{label}: {detected_text}"
                else:
                    label_with_text = label

                conf_text = f"{label_with_text} ({conf_val:.2f})"  # Text for label and confidence
                ...
                cv2.rectangle(image_filtered_detections, (x1_orig, label_y - th), (x1_orig + tw + 4, label_y + 4),
                              color, -1)
                cv2.putText(image_filtered_detections, conf_text, (x1_orig + 2, label_y + 2), cv2.FONT_HERSHEY_SIMPLEX,
                            0.6, (0, 0, 0), 2)

                detection_count += 1
                all_detected_labels.append(label_with_text)  # Add detected label to the list
        ...
        label_counts = Counter([label.split(':')[0] for label in all_detected_labels])  # Count base labels without text
        ...
        with open(csv_path, mode='a', newline='') as file:
            ...
            if not file_exists:  # If the file doesn't exist, write the header
                writer.writerow([
                    ...
                    "detected_classes", "detected_texts",
                    "total_detected_classes", "percentage_detected",
                    "conf_threshold"
                ])

            # Combine all detected texts
            detected_texts = "; ".join(
                [f"{d['label']}: {d['detected_text']}" for d in detection_json_data if d['detected_text']])
...
def main():
    parser = argparse.ArgumentParser(
        description="YOLOv8 Webcam or Image Inference Tool with OCR")  # Create argument parser
    ...
    parser.add_argument("--no-show", action="store_true",
                        help="Disable image preview (headless mode)")  # Disable preview
    parser.add_argument("--cam-width", type=int, default=1024, help="Camera width")  # Camera width
    parser.add_argument("--cam-height", type=int, default=1024, help="Camera height")  # Camera height
