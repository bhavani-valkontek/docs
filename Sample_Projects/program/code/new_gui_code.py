...
model_btn = tk.Button(frame, text="Load YOLO Model", command=load_model)
model_btn.grid(row=0, column=0, padx=5, pady=5)

model_label = tk.Label(frame, text="Model: None", anchor='w')
model_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

image_btn = tk.Button(frame, text="Load Image", command=load_image)
image_btn.grid(row=1, column=0, padx=5, pady=5)

run_btn = tk.Button(frame, text="Run Detection", command=run_detection)
run_btn.grid(row=1, column=1, padx=5, pady=5)

confidence_slider = tk.Scale(frame, from_=0.1, to=1.0, resolution=0.05, orient=tk.HORIZONTAL, label="Confidence Threshold")
confidence_slider.set(0.5)
confidence_slider.grid(row=2, column=0, columnspan=2, padx=5, pady=5)

zoom_rotate_frame = tk.Frame(root)
zoom_rotate_frame.pack(pady=5)

tk.Button(zoom_rotate_frame, text=" Zoom In", command=zoom_in).grid(row=0, column=0, padx=5)
tk.Button(zoom_rotate_frame, text=" Zoom Out", command=zoom_out).grid(row=0, column=1, padx=5)
tk.Button(zoom_rotate_frame, text="⟲ Rotate Left", command=rotate_left).grid(row=0, column=2, padx=5)
tk.Button(zoom_rotate_frame, text="⟳ Rotate Right", command=rotate_right).grid(row=0, column=3, padx=5)
